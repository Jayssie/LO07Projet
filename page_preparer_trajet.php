...
if ((isset($_POST)) and ( !empty($_POST))) {
    ...
    $requete3 = "UPDATE trajet SET statut = 'preparation' WHERE t_id = $t_id";
    $resultat3 = mysqli_query($DataBase, $requete3);
    $requete37 = "UPDATE reservation SET statut = 'preparation' WHERE trajet_id = $t_id";
    $resultat37 = mysqli_query($DataBase, $requete37);

    $requete99 = "select passager_id from reservation where trajet_id=$t_id";
    $resultat99 = mysqli_query($DataBase, $requete99);
    $requete88 = "select user_id,ville_depart,ville_arrive,depart_date,depart_time from trajet where t_id=$t_id";
    $resultat88 = mysqli_query($DataBase, $requete88);
    if ($resultat99) {
        $ligne88 = mysqli_fetch_array($resultat88, MYSQLI_NUM);
        while ($ligne99 = mysqli_fetch_array($resultat99, MYSQLI_NUM)) {
            $date=date('d-m-Y H:i:s');
            $requete77 = "insert into messagerie values(0,$ligne88[0],$ligne99[0],'On vous informe que votre trajet de $ligne88[1] a $ligne88[2] le $ligne88[3] a $ligne88[4] est pret','$date')";
...
//                window.location.href = 'page_effectuer_trajet.php';
            }
...
//                window.location.href = 'page_effectuer_trajet.php';
            ...
            function checksup(input) {
                ...
                document.getElementById('formID').submit();
//                window.location.href = 'page_supprimer_trajet.php';
            }
        </script>
    </body>
</html>